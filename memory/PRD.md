...
- **Mar 17, S9**: RFP Generator feature — new Studio card, 4-step wizard (upload template → configure → loading → preview), batched AI generation (2 sections per call), template parsing (PDF/DOCX/TXT), sample template fallback, Export dropdown (Word DOCX + PDF) with professional formatting (enhanced title page with teal/gold theme, proper Heading 1 styles, TOC with numbered entries, citations stripped from all outputs, all sections including Evaluation/Submission have content), Copy/Save/Regen/Print, EN/AR translations.

...
export default function KnowledgeBase() {
  ...
  const downloadOutput = useCallback(async (out) => {
    ...
    // For infographic, generate AI images and render Canvas PNG
    if (out.type === "infographic" && out.slides_data && out.slides_data.length > 0) {
      try {
        await renderInfographicWithImages(out, API_URL, toast);
        return;
      } catch (e) {
        console.error('Infographic generation error:', e);
        toast(`Infographic export failed: ${e.message || 'Unknown error'}`, "error");
      }
    }
